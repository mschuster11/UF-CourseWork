...
; Description:  This assembly application filters data from a given input table 
;               based on the constraints listed in Lab0's lab doc.
...
START:
    clr currentVal                ; Initialize the register holding each table value to zero.
    ldi ZL, byte3(inputTable << 1); Temporarily store the most significant byte on the input table's address in ZL.
    out CPU_RAMPZ, ZL             ; Load the upper third byte into the left most byte of the Z pointer.
